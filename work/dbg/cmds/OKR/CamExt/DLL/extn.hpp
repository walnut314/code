...
public:
    ...
    HRESULT Eval(
        ...
        __try {
            if ((Status = m_DebugControl->SetExpressionSyntax(DEBUG_EXPR_CPLUSPLUS)) != S_OK) {
                return Status;
            }
            if ((Status = m_DebugControl->Evaluate(expr, DEBUG_VALUE_INVALID, val, NULL)) != S_OK) {
                return Status;
            }
            if ((Status = m_DebugControl->SetExpressionSyntax(DEBUG_EXPR_MASM)) != S_OK) {
                return Status;
            }
    ...
    QueryInterfaces(
        ...
        HRESULT Status = S_OK;
        if ((Status = DebugClient->QueryInterface(__uuidof(IDebugControl), (PVOID *)&m_DebugControl)) != S_OK) {
            return Status;
        }
        if ((Status = DebugClient->QueryInterface(__uuidof(IDebugSymbols), (PVOID *)&m_DebugSymbols)) != S_OK) {
            return Status;
        }
        if ((Status = DebugClient->QueryInterface(__uuidof(IDebugDataSpaces), (PVOID *)&m_DebugDataSpaces)) != S_OK) {
            return Status;
        }
        return Status;
    }
