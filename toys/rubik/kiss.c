...
List *list_create(int key, const char *value, void *ctx)
{
...
void list_append(List *head, int key, const char *value, void *ctx)
{
    List *n = list_create(key, value, ctx);
    List *tmp = head->next;
    List *prev = head;
...
List *list_find(List *head, int key, const char *value)
{
    if (!head) return NULL;
    List *n = head->next;
    while (n) {
...
int hash_find(const char *key, int value)
{
    int hval = _hash(key);
...
void pickle_test()
{
    ...
    o.member3 = 44;
    o.member4 = 45;
    pickle("test.bin", (void *) &o, sizeof(Object));
    Object o2;
    unpickle("test.bin", (void *) &o2, sizeof(Object));
    show(&o2);
}
